...
def km2mile(x):
	""" a function to convert km to mile (x* 0.621371)""" 
	return int(x)

def calc_dist(lat1, lon1, lat2, lon2):
	""" a function to calcutale the distance in miles between two
	points on the earth, given their latitudes and longitudes in degrees"""
	# LAX
	#lat2 = math.radians(33.9425)
	#lon2 = math.radians(118.4072)
	# JFK
	#lat2 = math.radians(40.6397)
	#lon2 = math.radians(73.7789)

	# JFK -> LAX is 2467 miles ICN -> KBV 2657km
...
def formview(request):
	""" A view for the main form in which the user must enter two airports to 
	see https://docs.djangoproject.com/en/dev/topics/forms/ """

	# If the form has been submitted...
	airportlists = Airport.objects.all()
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			if (Airport.objects.filter(code=code1).exists() and Airport.objects.filter(code=code2).exists()):
				...
				return render(request, 'caldist/out.html', {'distance': mydist})

			# if not, go to "fail" page
			else:
				# Redirect to fail page after POST
				return HttpResponseRedirect('caldist/fail/')
	...
	# pass variables: form
	return render(request, 'caldist/formtemplate.html', {'form': form, 'airportlists': airportlists})
